...
Dupliquez le fichier d'exemple et renommez-le en .env en lançant cette commande dans votre terminal :

cp .env.example .env

(Vous pouvez ouvrir le fichier .env nouvellement créé pour y définir vos propres identifiants si vous le souhaitez).

2. Lancement de la base de données MongoDB
Démarrez le service de base de données en arrière-plan avec la commande suivante :
...
Une fois l'exécution terminée avec succès, le conteneur du script s'arrêtera automatiquement (Exited 0) pour libérer les ressources de la machine, tandis que la base MongoDB restera active pour vos consultations.
...
🧹 Arrêt et nettoyage
Pour éteindre la base de données et nettoyer les ressources Docker de votre PC, lancez :
